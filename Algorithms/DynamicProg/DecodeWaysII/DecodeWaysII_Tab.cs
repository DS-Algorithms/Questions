...
public class Solution
{
    ...
    public int NumDecodings(string s)
    {
        ...
        {
            int k = i - 1; // index for s is one behind dp index 'i'
            ...
            if (s[k - 1] == '*' && s[k] == '*')
            {
                result2 = dp[i - 2] * 15; // values 11 to 19 to 21 to 26 = 15
            }
            ...
            if (s[k - 1] == '*' && s[k] != '*')
            {
                ...
                int.TryParse(s[k].ToString(), out var val);
            ...
            if (s[k] == '*')
            {
                if (s[k - 1] == '1')
                    ...
                    result2 = dp[i - 2] * 6;
                else if (s[k - 1] != '*') // first digit is greater than 2
                    result2 = 0;
            }
            ...
            if (s[k - 1] != '*' && s[k] != '*')
            {
                int.TryParse(s.Substring(k - 1, 2), out var val);
                if (val >= 10 && val <= 26)
                    result2 = dp[i - 2];
                else
                    result2 = 0;
